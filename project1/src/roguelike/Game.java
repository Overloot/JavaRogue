...
private int sign(int x){
	...
	public void TryToOpenSomething(boolean isPlayer, int ny, int nx, boolean mode){
		...
		if (map.field[ny][nx].getOpened() == mode){
			if (isPlayer)
			if (mode)
...
void CheckTimeEffects(){
	boolean x = false;
	while (Player.getXP() >= Game.MAX_EXP) {x = true; LevelUp();}
	if (x){
...
public void SwitchMap(int dmap){
	...
	if (Maplist[newnum] == null){
	    ...
	    if (CurrentMapNumber == MAX_FLOORS - 1) LogMessage("ВЫ ДОБРАЛИСЬ ДО ПОСЛЕДНЕГО УРОВНЯ И ВЫИГРАЛИ ИГРУ! СПАСИБО ЗА ТЕСТИРОВАНИЕ!");
...
public void TryToLookAtMonster(int y, int x){
	...
	if (map.field[y][x].getItemList().size()!=0){
