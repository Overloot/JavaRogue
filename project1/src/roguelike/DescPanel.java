...
class DescPanel extends JPanel {
    ...
    public void DrawItemDesc(Graphics g) {
        ...
        if (so.STR_UP.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# вашу #8#силу.#^#", leftX, strY);
        else if (so.STR_UP.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# вашу #8#силу.#^#", leftX, strY);

        if (so.AGI_UP.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# вашу #8#ловкость!#^#", leftX, strY);
        else if (so.AGI_UP.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# вашу #8#ловкость.#^#", leftX, strY);

        if (so.END_UP.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# вашу #8#выносливость!#^#", leftX, strY);
        else if (so.END_UP.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# вашу #8#выносливость.#^#", leftX, strY);

        if (so.LUCK_UP.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# вашу #8#удачу!#^#", leftX, strY);
        else if (so.LUCK_UP.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# вашу #8#удачу.#^#", leftX, strY);

        if (so.DFIRE.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваш урон #2#огнем.#^#", leftX, strY);
        else if (so.DFIRE.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваш урон #2#огнем.#^#", leftX, strY);

        if (so.DCOLD.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваш урон #4#холодом.#^#", leftX, strY);
        else if (so.DCOLD.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваш урон #4#холодом.#^#", leftX, strY);

        if (so.DELEC.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваш урон #5#электричеством.#^#", leftX, strY);
        else if (so.DELEC.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваш урон #5#электричеством.#^#", leftX, strY);

        if (so.DPOISON.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваш урон #3#ядом.#^#", leftX, strY);
        else if (so.DPOISON.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваш урон #3#ядом.#^#", leftX, strY);

        if (so.DNORMAL.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваш #8#урон.#^#", leftX, strY);
        else if (so.DNORMAL.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваш #8#урон.#^#", leftX, strY);

        if (so.RFIRE.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваше сопротивление к #2#огню.#^#", leftX, strY);
        else if (so.RFIRE.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваше сопротивление к #2#огню.#^#", leftX, strY);

        if (so.RCOLD.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваше сопротивление к #4#холоду.#^#", leftX, strY);
        else if (so.RCOLD.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваше сопротивление к #4#холоду.#^#", leftX, strY);

        if (so.RPOISON.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваше сопротивление к #3#яду.#^#", leftX, strY);
        else if (so.RPOISON.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваше сопротивление к #3#яду.#^#", leftX, strY);

        if (so.RELEC.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваше сопротивление к #5#электричеству.#^#", leftX, strY);
        else if (so.RELEC.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваше сопротивление к #5#электричеству.#^#", leftX, strY);

        if (so.RNORMAL.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# ваше сопротивление к #8#урону.#^#", leftX, strY);
        else if (so.RNORMAL.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# ваше сопротивление к #8#урону.#^#", leftX, strY);

        if (so.SW_UP.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#увеличить#^# размер вашего инвентаря.", leftX, strY);
        else if (so.SW_UP.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#уменьшить#^# размер вашего инвентаря.", leftX, strY);


        if (so.BLINK)
            drawColorString(g, "Этот предмет может #8#телепортировать вас на короткое расстояние.#^#", leftX, strY);
        ...
        if (so.FOVRAD.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#улучшить#^# ваше #8#зрение!#^#", leftX, strY);
        else if (so.FOVRAD.getCurrent() < 0)
            drawColorString(g, "Этот предмет может #2#ухудшить#^# ваше #8#зрение!#^#", leftX, strY);

        if (so.PARALYZECOUNT.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #5#парализовать#^# вас!", leftX, strY);

        if (so.HEALPOISON.getCurrent() > 0)
            drawColorString(g, "Этот предмет может #3#исцелить#^# вас от #3#яда!#^#", leftX, strY);
