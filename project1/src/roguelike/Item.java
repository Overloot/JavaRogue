...
public class Item {
    ...
    public Item(BaseItem bm, int y, int x, Map map, Game game) {
        ...
        if (type == Itemset.TYPE_MELEE_WEAPON)
            generateWeapon(this);
        else if (type == Itemset.TYPE_POTION && Itemset.ID_ITEMS[id] == 0) {
            real_name = name;
            name = "Непонятное зелье";
            identify = false;
        } else if (type == Itemset.TYPE_SCROLL && Itemset.ID_ITEMS[id] == 0) {
            real_name = name;
    ...
    public static void generateArmor(Item item) {
        ...
        if (item.game.dice(chanse_to_have_stats_for_armor, 100)) {
            ...
            item.identify = false;
            item.name += "(?)";
        } else if (item.game.dice(chanse_to_have_resists_for_armor, 100)) {
    ...
    public static void generateWeapon(Item item) {
        ...
        if (item.game.dice(chanse_to_have_damages_for_weapon, 100)) {
            ...
            item.identify = false;
            item.name += "(?)";
        } else if (item.game.dice(chanse_to_have_stats_for_weapon, 100)) {
            ...
            item.identify = false;
            item.name += "(?)";
        } else if (item.game.dice(chanse_to_have_resists_for_weapon, 100)) {
    ...
    public void setIdentify(boolean b) {
        identify = b;
    }
